...
unc_AV_data <- filter(unc_AV_data, temperature == 20) #filters out the 25 degree data
unc_AV_data$genotype <- factor(unc_AV_data$genotype, levels =c("control","unc-33(mn407)", "unc-33(e204)", "unc-44(hrt-2)","unc-104(e1265)")) #reorders the genotypes so that they are organized in the proper way for visual presentation
...
Below is a section regarding penetrance of neurite AV presence. This isn't here because it's necessarily good data methodology, but it's just to see if the unc-104 results are similar to what was seen in the unc-104 paper by Stavoe et al 2016. Because according to my methods so far, they are not so similar

```{r}
unc_AV_data <- filter(unc_AV_data, temperature == 20)
neurite_penetrance_plot <-ggplot(data=unc_AV_data, aes(x=genotype))+
  geom_bar()+
  facet_wrap(vars(neurite_positive))
neurite_penetrance_plot
```
