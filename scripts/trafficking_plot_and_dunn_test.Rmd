...
bartlett.test(data = trafdat, avg_neurite_count_sum ~ genotype )#test for homogeneity of variance 
...
aov_residuals <- resid(one.way) #extracts residuals from linear model
shapiro.test(x=aov_residuals) #tests for normality of data set
bartlett.test(data = trafdat, avg_soma_count_sum ~ genotype ) #test for homogeneity of variance 
...
aov_residuals <- resid(one.way) #extracts residuals from linear model
shapiro.test(x=aov_residuals) #tests for normality of data set
bartlett.test(data = trafdat, avg_cell_count_sum ~ genotype ) #test for homogeneity of variance
...
fig_1_plotA <- ggplot(data=trafdat, aes(x=genotype, y=avg_neurite_count_sum)) +
  ...
  labs(y = "AV's per AIY Neurite",
       x = "Genotype")+
  geom_signif(comparisons = list(c('control','unc-33(mn407)' )), #creates the significance bars
              annotations = c("7.32e-05"))+
  geom_signif(comparisons = list(c('control','unc-33(e204)' )), #creates the significance bars
