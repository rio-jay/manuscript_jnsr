...
zData <-dunnTest(Neurite ~genotype, data = maturity_dat_red, method ="bh", list =TRUE) #performs a dunntest
zData<- zData$res #Takes out the stats info from the dunn test and creates a data frame
RedNeuriteComparison <-data.frame(comparison = zData[,1],
                                  z_value = zData[,2]
                                  )
RedNeuriteComparison #data frame of each comparison and z value in this region.
#Next, this will be cbinded to a data frame with the other regions to create the data frame with all of the info for the figure.
```
